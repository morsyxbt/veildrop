...
// Vercel Node function. POST saves a campaign, GET (?airdrop=) fetches one.
interface VReq {
  method?: string;
...
export default async function handler(req: VReq, res: VRes) {
  ...
  const out = await handleApi({ method: req.method ?? "GET", path: "/api/campaign", query, body: req.body });

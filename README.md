...
- **Distribute** - create a claim-based confidential airdrop (one shareable link) or push tokens directly (disperse)
- **Claim** - recipients connect a wallet, their drops are auto-detected, one click to claim, then decrypt for their eyes only
- **Create** - deploy a brand-new confidential token (owner-minted, optional max supply) or wrap any existing ERC-20 into a confidential version (and unwrap back)
- **Portfolio** - every confidential token you hold, one "Reveal all" to decrypt (only you can read them)
- **Campaigns** - track claim progress, extend the window, withdraw unclaimed funds - from any device, because admin rights live on-chain, not in the browser
...
| Recipient list before claiming | Absent | Encrypted entries keyed by address | Anyone with the claim link |
...
- **Recipient lists are confidential *on-chain*, not absolutely.** The hosted claim file is fetchable by anyone who has the claim link - that's what makes "one link for everyone" work. For maximum privacy on small lists, the app also generates a **self-contained link**: the whole campaign rides in the URL `#` fragment and never touches any server.
- **Store writes are creator-signed.** Saving a campaign or flagging it withdrawn requires an EIP-191 signature from the creator wallet, verified server-side - a third party can't overwrite a claim file or close someone else's drop. The store is a convenience layer: even if it disappeared, funds and claims are enforced entirely on-chain, and the sender can always refund.
- **The token registry** (`/api/token`, used for portfolio auto-listing and wrapper detection) stores public metadata only; always verify token addresses independently before wrapping meaningful value.
...
For a persistent backend in production, deploy `frontend/` to Vercel and set `UPSTASH_REDIS_REST_URL` + `UPSTASH_REDIS_REST_TOKEN` (see `frontend/.env.example`).

## Try it in five minutes

1. Connect a wallet on **Sepolia** (get test ETH from any Sepolia faucet).
2. **Faucet** - mint demo vUSD. Note your wallet shows no balance: it's confidential from the moment it's minted.
...
4. **Share the one link** - open it in another browser/wallet: only *that wallet's* allocation appears, as ciphertext. Claim it, then hit *Reveal my balance* - the amount decrypts off-chain, for your eyes only. Check the tx on Etherscan: no amount anywhere.
5. **Campaigns** - watch claim progress, extend the window, or refund whatever's unclaimed after it ends.
...
Tests cover the demo token (encrypted mint → user-decrypt round-trip, no cleartext on-chain, metadata) and the mintable token's bespoke logic (owner-only mint, cap enforcement including the exact-cap edge, uncapped mode, `totalMinted` accounting). Distribution logic is exercised through the live TokenOps contracts on Sepolia rather than re-tested here.

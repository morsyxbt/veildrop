...
// Vercel Node function. GET (?owner=) lists tokens a wallet created/wrapped.
interface VReq {
  method?: string;
...
export default async function handler(req: VReq, res: VRes) {
  ...
  const out = await handleApi({ method: req.method ?? "GET", path: "/api/tokens", query, body: req.body });
